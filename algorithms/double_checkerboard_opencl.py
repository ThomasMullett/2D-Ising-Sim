"""
algorithms.double_checkerboard_opencl.py

Contains the class for the double checkerboard spin pair implementation
"""
...
class DoubleCheckerboardOpenCLUpdater(IsingUpdater):
    def __init__(self, L: int, T: float, seed: int, J: float = 1, k: int = 1) -> None:
        ...
        self.kernel = self.prg.double_checkerboard

        # Precompute Boltzmann factors for ΔE = ±4J, ±8J
        self.beta = 1 / self.T
        boltz = np.exp(-self.beta * np.array([-8*self.J, -4*self.J, 0, 4*self.J, 8*self.J], dtype=np.float32))
        self.boltz_buffer = cl.Buffer(self.context, self.mf.READ_ONLY | self.mf.COPY_HOST_PTR, hostbuf=boltz)

        # Tile, local and global sizes
        self.t_size = 16
        self.g_size = (int(self.L/4), self.L)
        self.l_size = (int(self.t_size/2), self.t_size)
    ...
    # Perform one full Metropolis sweep using double checkerboard updates on GPU
    def sweep(self) -> None:
        # two sublattice updates (0=even, 1=odd)
        for color in (0, 1):
            # launch kernel
            self.kernel(
                self.queue, self.g_size, self.l_size,
                self.spin_buffer, self.rng_buffer,
                self.local_spin_cache, self.boltz_buffer,
                np.float32(self.beta),
                np.float32(self.J),
                np.int32(self.L),
                np.int32(self.t_size),
                np.int32(self.k),
                np.int32(color)
            ).wait()
    ...
    def _get_kernel_code(self) -> str:
        ...
        with open("algorithms/double_checkerboard.cl", "r") as file:
